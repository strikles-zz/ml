...
% recode yEnc = a matrix of 5000x10 which is derived from y (a matrix of size 5000 1)
% each row in yEnc has a value of 1 on the row indicated by each entry in y, and all 0s elsewhere
y_recode = eye(K)(y, :);
...
% this will effectively produce the double summation in the formula for J(theta)
% alternatively, can also use J = (1/m) * sum(sum(cost));
% here we get the unregularized cost function
J = (1/m) * sum(cost(:));
